...
function singUp() {
    $('#registration').click(function () {
        ...
        let vNanme = nameValidate(name);
        let vPhone = phoneValidate(phone);
        let vEmail = emailValidate(email);
        let vPassword = passwordValidate(password);
        let vAddress = addressValidate(address);

        if (vNanme && vPhone && vEmail && vPassword && vAddress) {
            let userData = {
                name: name,
                email: email,
                password: password,
...
function addressValidate(address) {
    if (address !== '') {
        if (address.length >= 5) {
            addSuccessSpan('.address-up', 'Адрес:');
            return true;
        } else {
            addErrorSpan('.address-up', 'Адрес должен содержать 5 и более символов!');
            return false;
        }
    } else {
...
function passwordValidate(password) {
    if (password !== '') {
        if (password.length >= 5) {
            addSuccessSpan('.pass-up', 'Пароль:');
            return true;
        } else {
            addErrorSpan('.pass-up', 'Пароль должен содержать 5 и более символов!');
            return false;
        }
    } else {
...
function phoneValidate(phone) {
    if (phone !== '') {
        if (phone[phone.length - 1] !== '_') {
            addSuccessSpan('.phone-up', 'Телефон:');
            return true;
        } else {
            addErrorSpan('.phone-up', 'Введите корректный номер телефона');
            return false;
        }
    } else {
...
function emailValidate(email) {
    if (email !== '') {
        if (isValidEmailAddress(email)) {
            addSuccessSpan('.email-up', 'Почта:');
            return true;
        } else {
            addErrorSpan('.email-up', 'Введите корректный почтовый адрес');
            return false;
        }
    } else {
...
function saveData(user) {
    ...
    $.ajax({
        ...
        if (typeof data.error !== 'undefined') {
            $('.error').remove();
            $('.name-up').before("<p class='error' style='color: red'>" + data.error + "</p>");
        }
    }).fail(function (err) {
        console.log(err);
    });
